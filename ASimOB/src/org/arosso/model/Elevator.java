...
public class Elevator extends Observable {
	...
	public void addCall(Passenger call) {
		...
		try {
			...
			AnnInputWVO.getInstance().addInputWVO(
					...
					(passengers.size()+1)/capacity, 
					model.getEstimatedTrafficIn(time), 
					model.getEstimatedTrafficOut(time),
					model.getEstimatedTrafficInterfloor(time), 
					direction, 
					call.getId());
			this.calls.add(call);
		} catch (Exception e) {
	...
	public void addPassenger(Passenger passenger) {
		...
		if(time2wait<30){
			normalizedTime = 0;
		}else if(time2wait>=30 && time2wait<50){
			normalizedTime = 0.25f;
		}else if(time2wait>=50 && time2wait<80){
			normalizedTime = 0.5f;
		}else if(time2wait>=90 && time2wait<140){
			normalizedTime = 0.75f;
		}else if(time2wait>140){
			normalizedTime = 1f;
		}
