...
public class TrafficInterFloorJReportGraph{
	...
	public Image getGraphic(){
        try {
			...
			db.query("SELECT ARRIVALTIME, ORIGINFLOOR, DESTINATIONFLOOR from PASSENGER");
			...
			for(int j=0;j<tam;j++){
				...
				int destinationFloor = Integer.parseInt(((Vector)registros.get(j)).get(2).toString());
				if( t_arribo<=(num_conjuntos*(300)) ){//Cada 5 minutos
					if(destinationFloor!=0 && originFloor!=0){
						num_pasajeros++;
					}
				}
				else{
