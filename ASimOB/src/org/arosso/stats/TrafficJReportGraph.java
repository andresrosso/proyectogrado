 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class TrafficJReportGraph{
	...
	public Image getGraphic(){
        try {
			...
			db.query("SELECT ARRIVALTIME from PASSENGER");
			...
			//HALLAMOS LA MEDIA
			media = sumatoriaX/num_conjuntos;
			System.out.print("Media: "+media);
			//HALLAMOS LA VARIANZA
			double sumatoriaV =0;
			for(int j=0;j<tam;j++){	
				double numerador = (sumatoriaX-media)*(sumatoriaX-media);
				sumatoriaV = sumatoriaV + (numerador/(num_conjuntos-1));
			}
			varianza = sumatoriaV;
